...
def clean(path):
    ...
    """
    proc = subprocess.Popen('git clean -qfdx'.split(),
...
def get_repo(path, origin=None):
    ...
    if not os.path.exists(path):
        if origin is None:
            raise Exception("%s does not exist and no remote URL was supplied" %
                             path)
...
def get_commit_before(repo, commit):
    ...
    for c in repo.iter_commits():
        ...
        raise Exception("No commit occurs before %s (@%d)" %
                        (commit, commit.authored_date))
